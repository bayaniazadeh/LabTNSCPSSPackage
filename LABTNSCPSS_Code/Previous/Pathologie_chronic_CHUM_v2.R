...
df <- fread("CHUM_Data/input_data_cleaned.csv", sep = ",", stringsAsFactors = FALSE)
...
codes_dict <- data.table::rbindlist(
  lapply(seq_len(nrow(codes_df)), function(i) {
    data.table(ICD10CAcode = codes_df$ICD10CAcodes[[i]], category = codes_df$category[i])
  })
)
...
assign_categories <- function(icd_codes) {
  ...
  icd_codes_dt <- data.table(ICD10CAcode = icd_codes_list)

  # Perform a join with codes_dict to find matching categories
  result <- merge(icd_codes_dt, codes_dict, by = "ICD10CAcode", all.x = TRUE)


  categories <- result[order(match(result$ICD10CAcode, icd_codes_list)), category]
...
find_basal_codes <- function(code_list, main_dt) {
  result <- main_dt[Comp_ICD10CAcodes %in% code_list, unique(B_ICD10CAcodesBasale)]
  if (length(result) == 0) {
...
file_path <- "CHUM_Data/updated_episodes_V2.csv"

write.csv(df_combined, file = file_path, row.names = FALSE)
